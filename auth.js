...
export function startDemo() {
  isDemoMode = true;
  S.user = DEMO.user; S.profil = DEMO.profil; S.firma = DEMO.firma;
  S.typen = DEMO.typen; S.kunden = DEMO.kunden; S.auftraege = DEMO.auftraege;
  S.inventar = DEMO.inventar; S.angebote = DEMO.angebote; S.rechnungen = DEMO.rechnungen;
...
export async function load() {
  ...
  const [a, b, c, dd, e, f, g] = await Promise.all([
  ...
  S.typen = a.data || []; S.kunden = b.data || []; S.auftraege = c.data || [];
  S.inventar = dd.data || []; S.angebote = e.data || []; S.rechnungen = f.data || []; S.nutzer = g.data || [];
}
...
export function render() {
  if (typeof window.appRender === 'function') { window.appRender(); return; }
  _renderPlaceholder();
}
...
window.authTab = (tab) => {
  ...
  ['a-err', 'r-err', 'r-ok'].forEach(id => { const el = document.getElementById(id); if (el) el.textContent = ''; });
};
...
window.doLogin = async () => {
  ...
  btn.textContent = 'Anmelden…'; btn.disabled = true; errEl.textContent = '';
  try {
    ...
    if (error) {
      errEl.textContent = error.message.toLowerCase().includes('invalid')
        ? 'Falsche E-Mail oder falsches Passwort.' : error.message;
    } else if (data?.user) { await boot(data.user); }
  } catch { errEl.textContent = 'Verbindungsfehler. Bitte nochmal versuchen.'; }
...
window.doReg = async () => {
  ...
  if (error) {
    errEl.textContent = error.message === 'User already registered'
      ? 'Diese E-Mail ist bereits registriert.' : error.message;
  } else if (isLocalMode && data?.user) { await boot(data.user); }
  else { okEl.textContent = 'Bestätigungsmail gesendet! Bitte E-Mail prüfen.'; }
};
...
window.doLogout = async () => {
  ...
  if (isLocalMode) { await localAuth.signOut(); S.user = null; S.profil = null; showAuth(); return; }
